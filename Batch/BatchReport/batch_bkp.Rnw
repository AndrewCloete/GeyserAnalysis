...
\title{Geyser report for \Sexpr{format(start_date, format="%B %d")} to \Sexpr{format(end_date, format="%B %d")}} % Title
...
Average daily number of events larger than \Sexpr{mean(b$event_midpoint[1])} \dotfill \Sexpr{round(mean(b$upper_events_count), 0)}\\
Average daily number of events smaller than \Sexpr{mean(b$event_midpoint[1])} \dotfill \Sexpr{round(mean(b$lower_events_count), 0)}\\
...
\begin{figure}[h!]
    \centering 
	\includegraphics[width=\textwidth]{./figs/water_and_energy_\Sexpr{geyser_id}.png}
...
\begin{figure}[h!]
    \centering 
	\includegraphics[width=\textwidth]{./figs/avg_temps_\Sexpr{geyser_id}.png}
    %\caption{ETSI horizontal architecture}
    %\label{fig:etsi-func}
\end{figure}
...
\begin{figure}[h!]
    \centering 
	\includegraphics[width=\textwidth]{./figs/est_cost_\Sexpr{geyser_id}.png}
    %\caption{ETSI horizontal architecture}
    %\label{fig:etsi-func}
\end{figure}
