...
source('~/Geyser/R/Daily/View/plot_daily_balloon.r')

events <- read_daily_events(date, this_geyser)

plot_daily_balloon(date, events, FALSE)

@

<<raw, fig.pos="h!", fig.width=15,fig.cap="Raw data", echo=FALSE, results='hide', warning=FALSE>>=
source('~/Geyser/R/Daily/View/plot_daily_raw.r')
source('~/Geyser/R/Daily/import_daily.r')
data = import_daily(date, this_geyser)
plot_daily_raw(data, FALSE)
@
